...
> 本文档面向：AI 服务开发/运维团队
>
> 目标：当客户接入时，如何将模拟服务替换为客户的真实 APS 系统，包括配置改动、接口适配、Dify 对接
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │  登录代理 / 会话管理                                   │   │
│  │  插单评估流程：触发 → 轮询 → 拉取结果 → 调 Dify         │   │
│  │  业务问答流程：转发 Dify /chat-messages               │   │
│  └─────────────────────────────────────────────────────┘   │
│                          │                                  │
│                          ▼                                  │
│                   Dify AI 服务                              │
...
DIFY_BASE_URL       = "http://139.224.228.33:8090/v1"  # ← Dify 服务地址
DIFY_API_KEY        = "app-NMXnjStbZL4uA5ufO7CCjOq7"  # ← 业务问答机器人 Key
DIFY_WORKFLOW_A_KEY = "app-20mcGFNUbrVe8UjiZkttDefU"   # ← 插单评估工作流 Key
```
...
| 2 | Dify 地址 | `ai_gateway.py` | Dify 服务地址（客户环境） |
| 3 | Dify API Key | `ai_gateway.py` | 业务问答机器人的 App Key |
| 4 | Dify Workflow Key | `ai_gateway.py` | 插单评估工作流的 App Key |
| 5 | 登录逻辑 | `ai_gateway.py` | 如果客户有自己的账号体系 |
| 6 | 接口路径 | `ai_gateway.py` | 根据客户 APS 的实际接口路径调整 |
| 7 | 字段映射 | `ai_gateway.py` | APS 返回字段 → Dify 输入字段 |
...
```python
# Dify 服务地址（包含 /v1 前缀）
DIFY_BASE_URL = "http://dify-host:8090/v1"

# 业务问答机器人 App Key
DIFY_API_KEY = "app-xxxxxxxxxxxx"

# 插单评估工作流 App Key
DIFY_WORKFLOW_A_KEY = "app-yyyyyyyyyyyy"
```

> **如何获取 Dify App Key**：登录 Dify → 进入应用 → API Key
...
插单评估结果会被传给 Dify Workflow，inputs 格式为：
...
Dify 内部会直接调用客户的 APS 查询接口，不需要经过网关。客户需要确保以下接口可被 Dify 服务器访问：

| 接口 | 说明 | Dify 调用方式 |
|------|------|--------------|
| `GET /aps/order` | 订单进度 | Dify HTTP 节点 |
| `GET /aps/shortage` | 缺料预警 | Dify HTTP 节点 |
| `GET /aps/capacity` | 产能负荷 | Dify HTTP 节点 |
| `GET /aps/risk` | 交期风险 | Dify HTTP 节点 |
...
### 7.1 业务问答机器人（Dify Chatbot）

**基本信息**：
- 应用类型：聊天助手
- API Key：在 Dify 中生成，填入 `DIFY_API_KEY`
...
### 7.2 插单评估工作流（Dify Workflow）

**基本信息**：
- 应用类型：工作流
- API Key：在 Dify 中生成，填入 `DIFY_WORKFLOW_A_KEY`
...
| AI 网关 | Dify | 8090 | 调用 Dify API |
| 客户 APS | AI 网关 | 8001 | 客户提供给前端的网关地址 |
| Dify | 客户 APS | 443/8080 等 | Dify 工具调用客户接口 |
...
AI 网关服务器 → Dify 服务器：允许 HTTP/HTTPS
Dify 服务器 → 客户 APS 服务器：允许 HTTP/HTTPS（用于 Dify 工具调用）
```
...
# 2. 确认 Dify 服务可达
curl --noproxy '*' http://dify-host:8090/v1/healthy

# 3. 确认 Dify API Key 有效
curl --noproxy '*' -H "Authorization: Bearer {DIFY_API_KEY}" \
     http://dify-host:8090/v1/chat-messages \
...
[CHAT] 正在转发到 Dify: http://dify-host:8090/v1/chat-messages
[CHAT] Dify 返回状态: 200
...
| Dify 返回 400 | App Key 无效 / 参数格式错误 | 检查 DIFY_API_KEY 和请求体 |
| Dify 返回 500 | Dify 内部错误 | 检查 Dify 日志 |
| 流式无 chunk 到达 | 代理缓冲了 SSE | 配置 no_proxy / 浏览器绕过代理 |
| 评估状态一直 running | APS 运算未完成 / 接口路径错误 | 检查 APS 的 warning 接口 |
| Dify 工具调用失败 | Dify 无法访问客户 APS | 检查 Dify → APS 网络通断 |
...
DIFY_BASE_URL=http://dify:8090/v1
DIFY_API_KEY=app-xxxxxxxxxxxx
DIFY_WORKFLOW_A_KEY=app-yyyyyyyyyyyy
```
